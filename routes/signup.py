    ...
    match "userName" in session:
        ...
        case False:
            ...
            if request.method == "POST":
                        ...
                        match userName.isascii():
                            case True:
                                ...
                                cursor.execute(
                                    f"""
                                    insert into
                                    users(userName,email,password,profilePicture,role,points,creationDate,creationTime)
                ...
                elif userName not in users and email in mails:
                    message("1", f'THIS EMAIL "{email}" IS UNAVAILABLE ')
                    flash("This email is unavailable.", "error")
                elif userName in users and email not in mails:
